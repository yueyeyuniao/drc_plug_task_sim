...
std::pair<tf::Vector3, tf::Vector3> wire;
...
class ImageConverter
{
  ...
  ros::Publisher point_cloud_wire_pub;
...
public:
    ...
    : it_(nh_)
  {
    ...
    point_cloud_wire_pub = nh_.advertise<PointCloud> ("/wire" , 10);
  ...
  void imageProcessing()
  {
    ...
    //   ros::spinOnce();
      ...
      //new
      std::vector<tf::Vector3> wire_plane_vector;
      tf::Vector3 wire_plane;
      //new
      ...
      for (int i = 0; i < pixel_size; i++){
        ...
        if (depth > 0.2){
          ...
          float co_z_temp = coordinate.z/1000-0.30;
          ...
          wire_plane.setX(co_x_temp);
          wire_plane.setY(co_y_temp);
          wire_plane.setZ(co_z_temp);
          wire_plane_vector.push_back(wire_plane);
          // new
         
        } 

      } 
        wire = svd_line_fitting_from_points(wire_plane_vector); //new
        // cout << "zip-tie-centroid-x" << wire.first.getX() << "zip-tie-plane-direction-x" << wire.second.getX() << endl;
        // new using best plane from points function to get the centrid and normal of the wire plane and calculate the frame of the zip-tie
        center_x = wire.first.getX();
        center_y = wire.first.getY();
        center_z = wire.first.getZ();

        direction_x = wire.second.getX();
        direction_y = wire.second.getY();
        direction_z = wire.second.getZ();
...
*/  
        ...
        br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "world", "wire"));
        for (int i = 0; i < 5000; i++)
          ros::spinOnce();

        target_pose.position.x = transform.getOrigin().x();
        target_pose.position.y = transform.getOrigin().y();
        target_pose.position.z = transform.getOrigin().z()+0.15;
        ...
        target_pose2.position.z = target_pose.position.z-0.15;

        // pose 3
        target_pose3 = target_pose2;
        target_pose3.position.y = target_pose2.position.y+0.1;
  ...
  void PointCloudPub()
  {
    ...
    for (int i=0; i<co_x.size();i++)
    {
      ...
      //msg->points.push_back (pcl::PointXYZ(center_x,center_y,center_z));
    }
    ...
    while (nh_.ok())
    {
      ...
      point_cloud_wire_pub.publish (msg);
      ros::spin();
      loop_rate.sleep ();
    }
