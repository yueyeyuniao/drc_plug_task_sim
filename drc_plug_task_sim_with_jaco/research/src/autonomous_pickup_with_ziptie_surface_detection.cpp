...
*/  
  ...
  tf::Transform tf_transform_calculation(float x, float y, float z)
  {
    ...
    listener.waitForTransform("/world", "/kinect2_rgb_optical_frame", ros::Time::now(), ros::Duration(3.0));
    listener.lookupTransform("/world", "/kinect2_rgb_optical_frame", ros::Time(0), stampedtransform_transform); // target frame to source frame
...
// new 
  tf::Quaternion tf_quaternion_calculation_new(tf::Transform transform)
  {   
    //calculate quaternion of the object
    tf::TransformListener listener_robot;
    tf::StampedTransform stampedtransform_quaternion;
    listener_robot.waitForTransform("/j2n6s300_end_effector", "/world", ros::Time::now(), ros::Duration(3.0)); // use "/j2n6s300_end_effector" if you want to keep the orientation of the endeffector
    listener_robot.lookupTransform("/j2n6s300_end_effector", "/world", ros::Time(0), stampedtransform_quaternion); // target frame to source frame
    tf::Transform nquaternion;
    nquaternion = stampedtransform_quaternion * transform;
    return nquaternion.getRotation();
  }
// new
