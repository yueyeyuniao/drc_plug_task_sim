...
public:
  ...
  void imageProcessing()
  {
    // while (ros::ok()){
    //   ros::spinOnce();
      for (int i=boundingbox_xmin; i<boundingbox_xmax; i++) { //959 x
        for (int j=boundingbox_ymin; j<boundingbox_ymax; j++){ //539 y
      ...
      for (int i = 0; i < pixel_size; i++){
        ...
        if (depth > 0.2){
          ...
          // ROS_INFO_STREAM(co_x[i] << co_y[i] << co_z[i] << ";");
          // cout << "i:" << i<< "xyz " << "x:" << co_x[i] << "y:"<< co_y[i] << "z:" << co_z[i] << ";" << endl;          
        } 
