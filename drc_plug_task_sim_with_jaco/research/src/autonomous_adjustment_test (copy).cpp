...
class PointCloudProc
{
  public:
    ...
    label:
        ...
        pass.setFilterLimits(center_object.y - 0.66, center_object.y + transform.getOrigin().y()-0.07); // 0.0 needs to change according to the ee's position
        pass.filter(*cloud_filtered_);
        pass.setInputCloud(cloud_filtered_);
    ...
    void CurveFit(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
    {
      ...
      //   isGrasp = true;
      // }
      if (filter_plug == false){
        if ((tip_temp +0.65) > 0.005)  
        {
