...
class PointCloudProc
{
  public:
    ...
    void pointcloudcb(const pcl::PointCloud<pcl::PointXYZ>::Ptr &msg)
    {
        ...
        pass.setFilterLimits(center_object.x - 0.05, center_object.x + 0.5);
        pass.filter(*cloud_filtered_);
        pass.setInputCloud(cloud_filtered_);
        pass.setFilterFieldName("y");
        pass.setFilterLimits(center_object.y - 0.55, center_object.y + 0.1); // 0.0 needs to change according to the ee's position
        pass.filter(*cloud_filtered_);
        pass.setInputCloud(cloud_filtered_);
    ...
    void CurveFit(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
    {
      ...
      // if roll = 0;
      // roll = 0;
      // pitch = atan((sqrt(pow((points.points[0].x-points.points[1].x),2)+pow((points.points[0].y-points.points[1].y),2)))/abs(points.points[0].z-points.points[1].z));
      // yaw = atan(-(points.points[0].x-points.points[1].x)/(points.points[0].y-points.points[1].y));
      // tf::Transform transform;
      // transform.setOrigin(tf::Vector3(points.points[0].x, points.points[0].y, points.points[0].z));
      // transform.setRotation(tf::createQuaternionFromRPY(roll, pitch, yaw));
      // br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "root", "wire_tip"));

      // if pitch = 0; ignore the rotation about the Y axis
      roll = atan((points.points[0].z-points.points[1].z)/(points.points[0].y-points.points[1].y));
      pitch = 0;
      yaw = atan((points.points[0].x-points.points[1].x)/sqrt(pow((points.points[0].y-points.points[1].y),2)+pow((points.points[0].z-points.points[1].z),2)));
      tf::Transform transform;
      transform.setOrigin(tf::Vector3(points.points[0].x, points.points[0].y, points.points[0].z));
      transform.setRotation(tf::createQuaternionFromRPY(roll, pitch, yaw));
      br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "root", "wire_tip"));
